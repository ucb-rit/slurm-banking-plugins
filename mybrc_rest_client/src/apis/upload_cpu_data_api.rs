...
pub trait UploadCpuDataApi {
    fn upload_cpu_data_update(&self, filename: &str, authorization: &str) -> Box<Future<Item = (), Error = Error<serde_json::Value>>>;
}
...
impl<C: hyper::client::Connect>UploadCpuDataApi for UploadCpuDataApiClient<C> {
    fn upload_cpu_data_update(&self, filename: &str, authorization: &str) -> Box<Future<Item = (), Error = Error<serde_json::Value>>> {
        ...
        let uri_str = format!("{}/upload_cpu_data/{filename}?{}", configuration.base_path, query_string, filename=filename);
